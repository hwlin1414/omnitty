...
static bool add_machines_from_file(const char *file, const bool fromcmd){
	char buf[128], rpath[128], cwd[128], *ptr, *ptr2;
	FILE *f;
	if (*file != '@' && fromcmd == false) return machmgr_add(file);

	if(*file == '@') file++;
	if ((f = fopen(file, "r")) == NULL) {
		snprintf(buf, 128, "Can't read file: %s", file);
		minibuf_msg(minibuf, buf, 0xF1);
		return false;
	}
	minibuf_put(minibuf, "Adding machines from file...", 0x70);
	...
	*(ptr + 1) = '\0';
	...
	while ((ptr = fgets(buf, 128, f)) != NULL){
		while(*ptr == ' ' || *ptr == '\t') ptr++;
		if(*ptr == '#') continue;
		ptr2 = ptr;
		while(*ptr2 != '\0' && *ptr2 != '\n' && *ptr2 != '\r' && *ptr2 != '#') ptr2++;
		*ptr2 = '\0';
		...
		if(*ptr == '\0') continue;
		if(add_machines_from_file(ptr, false) == false){
			fclose(f);
			return false;
		}
	}
...
static void add_machine() {
   ...
   *buf = 0;
   if (minibuf_prompt(minibuf, "Add: ", 0xE0, buf, 32)) {
	   add_machines_from_file(buf, false);
   }
}
...
int main(int argc, char **argv) {
   ...
   */
   ...
   for(i = 1; i < argc; i++) add_machines_from_file(argv[i], true);
   while (!quit) {
      if (zombie_count) {
         zombie_count--;
         chldpid = wait(NULL);
