...
#define CMD_SSH_FORMAT "/usr/bin/ssh %s"
#define CMD_TELNET_FORMAT "/usr/bin/telnet %s"
...
Machine *machine_new(const char *name, int vtrows, int vtcols) {
   ...
   if (strncmp(name, "telnet:", 7) == 0){
      m->name = strdup(name + 7);
      if (120 < snprintf(cmd, 120, CMD_TELNET_FORMAT, m->name)) abort();
   }else{
      m->name = strdup(name);
      if (120 < snprintf(cmd, 120, CMD_SSH_FORMAT, m->name)) abort();
   }
